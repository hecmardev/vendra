...
- **Body Font:** Manrope
- **Mood:** modern, geometric, clean, confident, tech-forward, marketplace
- **Notas:** Familia única (variable font) para títulos y cuerpo. Peso 700/800 en
  títulos, 400/500 en cuerpo, 600 en labels/CTA. Auto-hospedada vía `next/font`.
- **Implementación:** `next/font/google` → `Manrope` con `variable: '--font-manrope'`
  en `app/layout.tsx`; expuesta como `font-sans` por defecto en `tailwind.config.ts`.
  (No se usa `@import` de Google Fonts: next/font evita el request externo.)
